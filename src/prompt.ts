...
entryCondition MUST reflect the DO NOT FEATURE guardrail above: triggerPrice must be near current price or a genuinely near-term plausible level — never a distant pullback level that may never occur. entryCondition.type is "IMMEDIATE" only for a true enter-right-now ACTIONABLE NOW pick; otherwise match PULLBACK/BREAKOUT/RECLAIM to what "ENTRY" actually says. validityWindowMinutes: 20-45 minutes for an ACTIONABLE NOW entry near current price, up to 120-240 minutes for a TRIGGER WATCH confirmation — never longer than it should realistically take for the stated condition to either happen or be dead, since a distant window on an implausible level is exactly the failure mode being fixed here.
**Reachability check (measured live: this was silently failing — WATCH-tier entries requiring 20-40%+ moves within their window never triggered once across dozens):** triggerPrice should require no more than roughly a 15% move from entrySnapshot.priceUsd for a 20-45 minute window, or roughly 25% for a 120-240 minute TRIGGER WATCH window — a bigger, slower memecoin move can still take longer than that to actually resolve. If the level you'd naturally want is further away than fits its window, do NOT invent a distant trigger to force a trade plan — either widen entryCondition.type to something reachable (e.g. a closer PULLBACK level instead of a distant RECLAIM), or drop the token to NO TRADE/AVOID instead. An unreachable trigger that just expires unfilled is worse than no trade plan at all.
structuralInvalidation.price must match the visible INVALIDATION level exactly. targets must include at least TP1. If both ACTIONABLE NOW and TRIGGER WATCH were empty, emit ---TRADEPLAN--- followed by [].
