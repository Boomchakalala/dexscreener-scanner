...
{"symbol": "...", "tokenAddress": "...", "poolAddress": "...", "tier": "FLASH", "entrySnapshot": {"priceUsd": number, "marketCapUsd": number, "liquidityUsd": number}, "entryCondition": {"type": "IMMEDIATE"|"PULLBACK"|"BREAKOUT"|"RECLAIM", "triggerPrice": number|null, "description": "...", "validityWindowMinutes": number}, "structuralInvalidation": {"price": number, "description": "..."}, "targets": [{"label": "TP1"|"TP2", "price": number, "note": "..."}], "thesis": "one sentence restating why this is flashing", "confidence": "LOW"|"MEDIUM"|"HIGH"}

entryCondition.type should be "IMMEDIATE" (with triggerPrice null) in the large majority of flags — confirmed live: a flagged token whose plan waited for a reclaim instead of buying now sat unfilled and never triggered while the move played out and reversed within minutes. The code already guards against chasing (an IMMEDIATE fill gets rejected as MISSED if price already ran more than 15% past this snapshot before the next check), so you do not need to manually add a pullback condition as a safety net — that guard is the safety net. Only use PULLBACK/BREAKOUT/RECLAIM when your own Trade plan line explicitly called entering now reckless, which should be rare. validityWindowMinutes should be short (10-20 minutes for SPIKE, up to 30-45 for MOMENTUM given it moves a bit slower) — do not reuse the deep scan's longer windows. structuralInvalidation.price and targets must match the prose Invalidation/Targets lines exactly.

confidence sizes the position — every flash alert used to get the identical size regardless of how good the setup actually was, which this replaces. Judge it on the same evidence you already weigh, not a separate exercise:
HIGH: clean buy-dominant flow across multiple windows (not just one vertical 5-minute candle), real holder count and organicScore backing it, no wash-trading or factory-deployer red flags, a genuine structural thesis (an actual base/reclaim, not just "it's going up") — the kind of setup your own prose would call clean or strong.
MEDIUM: a normal, solid flash-worthy setup — real evidence, nothing exceptional, nothing concerning. Default here when genuinely unsure between MEDIUM and either neighbor.
LOW: thinner evidence that still clears the bar for a flash alert — mostly momentum/price action with weak holder or organic-activity backing, some wash-trading or extension risk, or a thesis you'd hedge in prose ("could be starting to move, but..."). Still worth flagging given flash is speed-first, just don't size it like a clean HIGH setup.
If you output NOTHING, emit ---TRADEPLAN--- followed by [].`;
